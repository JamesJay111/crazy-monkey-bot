...
1. **`src/config/env.ts`** - 环境变量配置
   - 新增：`POLL_INTERVAL_MS` (number, 默认 300000 = 5 分钟)
   - 新增：`FUNDING_THRESHOLD_DECIMAL` (number, 默认 -0.0005 = -0.05%)
   - 新增：`DAILY_TWEET_LIMIT` (number, 默认 3)

2. **`src/services/fundingNegativeOIService.ts`** - 资金费率筛选服务
   - 修改：`filterNegativeFunding()` - 使用阈值触发（fundingRate <= -0.0005）
   - 新增：`normalizeFundingRateToDecimal()` - 归一化资金费率到 decimal 形式
   - 修改：`pickMaxOI()` - OI 相同时按 symbol 字母序排序

3. **`src/services/tweetContent.service.ts`** - 推文内容生成服务
...
4. **`src/services/xAutoTweetJob.service.ts`** - 自动发推任务服务
   - 修改：`start()` - 从 8h 固定间隔改为可配置轮询（POLL_INTERVAL_MS）
   - 修改：`runTweetJobOnce()` - 添加每日限额检查
   - 修改：`fetchHistoricalData()` - 历史数据使用 4h interval
   - 修改：`shouldSkipTweet()` - 48 小时去重逻辑（保持）
   - 修改：`loadTweetState()` - 支持 daily 字段
   - 修改：`saveTweetState()` - 支持 daily 字段，48h bucket
...
**规则**：
- 如果绝对值 > 1，视为百分数形式（例如 -0.05 表示 -0.05%），除以 100
- 如果绝对值 <= 1，视为 decimal 形式（例如 -0.0005 表示 -0.05%），直接返回
...
**默认值**：`300000` (5 分钟 = 300000 毫秒)

**可配置方式**：
```env
POLL_INTERVAL_MS=300000  # 5 分钟（默认）
# 或
POLL_INTERVAL_MS=600000  # 10 分钟
```
...
**位置**：`src/services/xAutoTweetJob.service.ts` 的 `TweetState` 接口

```typescript
interface TweetState {
  lastTweetAt: number;
  lastTweetTicker: string | null;
  lastTweetKey: string | null;
  daily: {
    date: string;      // YYYY-MM-DD (Asia/Bangkok 时区)
...
**位置**：`checkDailyQuota()` 和 `incrementDailyQuota()` 方法

**规则**：
1. 每次检查时，如果 `state.daily.date !== today` → 重置为 `today + count=0`
2. 如果 `count >= DAILY_TWEET_LIMIT` → 跳过发送
3. 仅在"真实发送成功"后 `count += 1`
4. `PREFLIGHT_MODE=true` 不增加 count（因为不是真实推送）
...
private getTodayDate(): string {
  ...
  const day = String(bangkkokTime.getDate()).padStart(2, '0');
