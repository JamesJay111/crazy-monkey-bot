 ...
 * 
 * 功能：
 * - 每 2 小时扫描一次 CoinGlass 新闻（经济数据、财经事件、央行动态、新闻和快讯）
 * - 只要有新增新闻就推送
 * - 推文格式：带标签 [经济数据]/[财经事件]/[央行动态]/[新闻和快讯]
 * - 调用 DeepSeek 生成解读（关于加密货币宏观市场影响）
 * - 推送到 Twitter 三账户（中文/英文/韩语）
 */
...
export class MacroNewsPushService {
  private intervalId: NodeJS.Timeout | null = null;
  private isRunning = false;
  private readonly POLL_INTERVAL_MS = 2 * 60 * 60 * 1000; // 2 小时
  private readonly TIME_WINDOW_MS = 2 * 60 * 60 * 1000; // 获取过去 2 小时内的新闻
  private readonly TEST_WEBHOOK_URL = 'https://open.larksuite.com/open-apis/bot/v2/hook/f182517d-8c87-4a09-adc9-be40730b0506';
  private useTestMode: boolean = false; // 测试模式：推送到 Webhook 而不是 Twitter
  ...
  constructor(
    ...
    this.useTestMode = options?.testMode ?? false;
    this.initDatabase();
  }
  ...
  private initDatabase(): void {
    try {
      ...
      `);
      logger.info('Macro news push log table initialized');
    } catch (error) {
      logger.error({ error }, 'Failed to init macro news push log database');
    }
  }

  start(): void {
    if (this.intervalId) {
      logger.warn('Macro news push service is already running');
      return;
    }
    ...
    }, 'Starting macro news push service');
    ...
    logger.info('Macro news push service started');
  }
  ...
  stop(): void {
    if (this.intervalId) {
      ...
      logger.info('Macro news push service stopped');
    }
  }

  private async runScanOnce(): Promise<void> {
    if (this.isRunning) {
      logger.warn('Macro news scan is already running, skipping');
      return;
    }
    ...
    try {
      logger.info('Running macro news scan...');
      ...
      for (const news of newNews) {
        try {
          ...
          logger.error({ error, newsId: news.id, newsType: news.type }, 'Failed to push news');
        }
      }

      logger.info({ pushedCount: newNews.length }, 'Macro news scan completed');

    } catch (error) {
      logger.error({ error }, 'Macro news scan failed');
    } finally {
  ...
  private async fetchAllNews(): Promise<NewsItem[]> {
    ...
    try {
      ...
      for (const article of articles) {
        if (article.article_id && article.title && article.publish_time) {
          allNews.push({
      ...
      for (const newsflash of newsflashes) {
        if (newsflash.newsflash_id && newsflash.title && newsflash.publish_time) {
          allNews.push({
            id: `newsflash-${newsflash.newsflash_id}`,
            type: 'newsflash',
            title: newsflash.title,
            content: newsflash.content,
            publishTime: newsflash.publish_time,
            url: newsflash.url,
      ...
      }, 'Fetched all news types');

    } catch (error) {
      logger.error({ error }, 'Failed to fetch all news');
    }
  ...
  private async pushNews(news: NewsItem): Promise<void> {
    try {
      ...
      const results = await this.publishTweets(news, tweets);
      ...
      }, 'News pushed successfully');

    } catch (error) {
      logger.error({ error, newsId: news.id }, 'Failed to push news');
      throw error;
    }
  }

  private async generateTweets(news: NewsItem): Promise<{ zh: string; en: string; ko: string }> {
    const zhTweet = await this.generateTweetForAccount(news, ACCOUNT_CONFIG.A);
  ...
  private async generateTweetForAccount(news: NewsItem, accountConfig: typeof ACCOUNT_CONFIG.A): Promise<string> {
    const language = accountConfig.language;
    ...
    if (baseTweet.length > 280) {
      const maxLength = 280;
      const titleAndTime = `${icon} ${label}${news.countryName ? ` | ${news.countryName}` : ''}\n\n${news.title}\n⏰ ${timeStr}`;
      const interpretationPart = interpretation ? `\n\n💡 ${interpretation}` : '';
      const availableLength = maxLength - titleAndTime.length - interpretationPart.length - 10;
      
      if (availableLength > 20 && background) {
        const truncatedBackground = background.substring(0, availableLength - 3) + '...';
        baseTweet = `${titleAndTime}${interpretationPart}\n\n📌 ${truncatedBackground}`;
      } else {
        baseTweet = `${titleAndTime}${interpretationPart}`;
      }
    }
...
背景：[背景信息]`;
    ...
    try {
      ...
      return {
        interpretation: interpretationMatch ? interpretationMatch[1].trim() : '关注市场动态',
...
- 确保翻译自然流畅`,
  ...
  private async publishTweets(
    ...
    // 测试模式：推送到 Webhook
    if (this.useTestMode) {
      return await this.publishToWebhook(news, tweets);
    }
    ...
    try {
      const resultC = await RetryUtil.retry(
        async () => {
          return await this.tweetService.sendTweet(tweets.ko, ACCOUNT_CONFIG.C.key);
        },
        { maxAttempts: 2, backoffMs: 2000, maxBackoffMs: 5000 }
      );
  ...
  /**
   * 测试模式：推送到 Lark Webhook（分开发送三条消息）
   */
  private async publishToWebhook(
    ...
    try {
      ...
      }, 'All news messages sent to webhook (test mode)');
  ...
  private async sendToWebhook(text: string): Promise<boolean> {
    try {
      ...
      const response = await axios.post(this.TEST_WEBHOOK_URL, payload, {
        headers: {
  ...
  private async logPush(
    ...
    try {
      ...
      logger.error({ error, newsId: news.id }, 'Failed to log push');
    }
  }
